...
def get_rows_by_id(cf_name, row_ids):
    ...
    try:
        return c.session.execute(
            SimpleStatement(query), consistency_level=ConsistencyLevel.ONE).result() 
    except InvalidRequest:
        raise NoSuchColumnFamilyException(cf_name)
    except IndexError:
...
def set_rows_data(cf_name, unique_ids, list_of_rows):
    ...
    try:
        c.session.execute(
            SimpleStatement(batch, consistency_level=ConsistencyLevel.ONE)
        ).result()
    except InvalidRequest:
        raise NoSuchColumnFamilyException(cf_name)
...
def get_row_spec_prop(cf_name, unique_id, cols):
    ...
    try:
        return c.session.execute(
            SimpleStatement(query,consistency_level=cassandra.ConsistencyLevel.ONE))[0] #consistency_level=cassandra.ConsistencyLevel.QUORUM
...
def delete_row(cf_name, unique_id):
    ...
    try:
        c.session.execute(
            SimpleStatement(query,consistency_level=cassandra.ConsistencyLevel.ONE)
        )
    except cassandra.InvalidRequest:
        raise NoSuchColumnFamilyException(cf_name)
    except IndexError:
